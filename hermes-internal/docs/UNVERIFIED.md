...
| 3 | Config-as-code is applied on every start path | `/etc/cont-init.d/*` only run on the s6 PID-1 path; on non-PID-1 platforms it may be skipped. **Addressed:** seeding now runs from BOTH the cont-init hook AND the CMD wrapper `scripts/entrypoint.sh` (which runs on every path), and the wrapper **hard-fails (exit 71) if the applied `config.yaml` does not match this image's config** — so the agent never starts silently on the default config. A marker `$HERMES_HOME/.hermes-internal-seeded` records the applied config hash. What remains unverified is only that, in a real container, the wrapper runs as CMD, `hermes` is on PATH, and the exec succeeds. | `docker compose up`; confirm a boot log line `[entrypoint] config verified (<hash>)`; break it deliberately (put a wrong `/opt/data/config.yaml`, restart) and confirm the container refuses to start | `scripts/entrypoint.sh`, `scripts/seed.sh`, `docker/cont-init.d/03-hermes-internal-seed`, `Dockerfile` CMD |
